...
LPA_wb_plus <- function(MATRIX, initialmoduleguess = NA) {
  ...
  if (is.na(initialmoduleguess))
    ...
    redlabels = sample(1:(initialmoduleguess + 1), dim(MATRIX)[1], replace =
                         TRUE)
  
  #Run Phase 1: Locally update lables to maximise Qb
  outlist = StageOne_LPAwbdash(row_marginals,
                               col_marginals,
                               MATRIX,
                               BMatrix,
                               Matsum,
  ...
  if (flipped == 1) {
    #If matrix was flipped, swap row and column labels
    holder = redlabels
  ...
  return(list(
    Row_labels = redlabels,
    Col_labels = bluelabels,
    modularity = Qb_now
  ))
}
...
WEIGHTEDMODULARITY <-
  function(BMatrix, Matsum, redlabels, bluelabels) {
    #see equation 8
    holdsum = 0
      ...
      if (NUMdiv > 1) {
        for (div1check in 1:(NUMdiv - 1)) {
          ...
          for (div2check in (div1check + 1):NUMdiv) {
            ...
            if (QQ > Qb_now) {
              #If a division will improve modularity - find the best way to do this
              FoundBetter = 0
              for (aa in 1:NUMdiv) {
              ...
              if (FoundBetter == 0) {
                #If no better configuration found - JOIN.
                redlabels = CHECK_RED
        ...
        if (CombinedDivisionsThisTime == 0) {
          #If no divisions were joined move on
          IterateFlag = 0
        }
      }
      else {
...
StageOne_LPAwbdash <-
  function(row_marginals,
    ...
    #Blue
    if (sum(is.na(bluelabels)) != BLUELABELLENGTH) {
      #occurs first time through as blue nodes unlabelled
    ...
    while (IterateFlag == 1) {
      ...
      for (bb in 1:length(bluelabels)) {
        ...
        for (ww in 1:length(bluelabelchoices)) {
          changebluelabeltest[ww] = sum((redlabels == bluelabelchoices[ww]) * MATRIX[, bb])  -  col_marginals[bb] *
            TotalRedDegrees[bluelabelchoices[ww]] / Matsum
        }
        
        #assign new label based on maximisation of above condition
        
        labels = which(changebluelabeltest == max(changebluelabeltest, na.rm =
                                                    TRUE))
      ...
      for (aa in 1:length(redlabels)) {
        ...
        for (ww in 1:length(redlabelchoices)) {
          changeredlabeltest[ww] = sum((bluelabels == redlabelchoices[ww]) * MATRIX[aa, ])  -  row_marginals[aa] *
            TotalBlueDegrees[redlabelchoices[ww]] / Matsum
        }
...
run_bivar_modules <- function(input){
  res <- DIRT_LPA_wb_plus(input)
  row_ix <- order(res$Row_labels)
  col_ix <- order(res$Col_labels)
  row_labels <- res$Row_labels[row_ix]
  col_labels <- res$Col_labels[col_ix]
  r = list(
    rix = row_ix,
    cix = col_ix,
    rl = row_labels,
