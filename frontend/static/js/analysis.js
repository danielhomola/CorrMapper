$(document).ready(function() {
    ...
    // we could do this from jinja, but it would screw up the form validation
    if (fs === 'False' || cols.length == 0 ){
        $("#fs_panel").hide();
    }
    ...
    // hide some panels on start
    $("#constrain_dist_panel").hide();
    conditional_fs();
    conditional_constrain_corr();

    // make conditional fields conditionally appear
    function conditional_fs(){
        if(document.getElementById('fs').checked) {
            $("#fs_cols").hide();
            $("#fs_method_panel").hide();
        } else {
            $("#fs_cols").show();
            $("#fs_method_panel").show();
        }
    }
    function conditional_constrain_corr (){
        if(document.getElementById('constrain_corr').checked) {
            $("#constrain_dist_panel").show();
        } else {
            $("#constrain_dist_panel").hide();
        }
    }

    $('#fs').click(conditional_fs);
    $('#constrain_corr').click(conditional_constrain_corr);
            ...
            request = $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data.status === "OK") {
                        hideErrorPanels();
                        if (guest == 1) {
                            $('#analyse-button-text').text('Sorry, guest cannot submit jobs')
                        }else{
