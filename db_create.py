...
with app.app_context():
    ...
    guest_role_exist = db.session.query(exists().where(Role.name == "superuser")).scalar()
    ...
    if not super_role_exist:
        ...
        db.session.commit()
    if not guest_role_exist:
        guest_user_role = Role(name='guest')
        db.session.add(guest_user_role)
        db.session.commit()
